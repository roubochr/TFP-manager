...
$Script:LogBufferSize = 10

# HUD dirty flag to skip unnecessary redraws
$Script:LastHudHash = ""

# Folder existence cache to reduce filesystem calls
$Script:FolderCache = @{}

# Adaptive polling for idle detection
$Script:ConsecutiveEmptyPolls = 0
...
function Log {
    ...
    # OPTIMIZATION: Buffered logging - collect logs and write in batches
    $Script:LogBuffer.Add($line)
    
    # Flush immediately on ERROR or WARN, or when buffer is full
    if ($level -eq "ERROR" -or $level -eq "WARN" -or $Script:LogBuffer.Count -ge $Script:LogBufferSize) {
        Flush-LogBuffer
    }
}
...
function Flush-LogBuffer {
    if ($Script:LogBuffer.Count -gt 0) {
        try {
            Add-Content -LiteralPath $LogFilePath -Value $Script:LogBuffer
...
function Get-FileHashBatch {
    ...
    # OPTIMIZATION: Parallel hash computation for large batches (PowerShell 7+ only)
    # Falls back to sequential for PowerShell 5.x
    
    if ($PSVersionTable.PSVersion.Major -ge 7 -and $Files.Count -gt 5) {
        try {
            $results = $Files | ForEach-Object -Parallel {
                try {
                    $hashObj = Get-FileHash -Algorithm SHA256 -LiteralPath $_.FullName -ErrorAction Stop
                    [PSCustomObject]@{
                        Path = $_.FullName
                        Hash = $hashObj.Hash
                    }
    ...
    # Sequential fallback for PowerShell 5.x or small batches
...
function Move-FileSafely {
    ...
    # OPTIMIZATION: Use cached folder creation
    try {
        New-DirectoryCached -Path $destDir -Force
    }
    catch {
        # Fallback: try direct creation if cache helper fails
...
function Test-DuplicateFile {
    ...
    # OPTIMIZATION: Lazy hashing - skip tiny files (< 512 bytes) to save CPU
    # These are usually metadata files, not actual STL files worth checking
...
function Show-UI {
    ...
    # STATIC HUD: Always clear and redraw at same position
    # This prevents any scrolling or positioning issues
    # The "static" part is that it appears in the SAME PLACE every time
    
    # OPTIMIZATION: Use faster Console API instead of Clear-Host
    try {
        [Console]::Clear()
        [Console]::CursorVisible = $false
    }
    catch {
        Clear-Host  # Fallback for environments without console access
    }
    ...
    # Footer with timestamp - compact (no newline - let countdown position naturally)
    Write-Host "  " -NoNewline
...
function Show-Countdown {
    ...
    # OPTIMIZATION: Adaptive polling - extend wait when idle
    $actualSeconds = $Seconds
    if ($Script:ConsecutiveEmptyPolls -gt 3) {
        $actualSeconds = [Math]::Min($Seconds * 2, 60)  # Max 60s when idle
    }

    $width = 30
    
    # OPTIMIZATION: Update twice per second for smoother countdown (but less CPU)
    $totalIterations = $actualSeconds * 2
    
    for ($iter = $totalIterations; $iter -gt 0; $iter--) {
        $secondsLeft = [Math]::Ceiling($iter / 2.0)
        
        if ($actualSeconds -eq 0) {
            $done = $width
        }
        else {
            $done = [int]( ($totalIterations - $iter) / [double]$totalIterations * $width )
        }
        if ($done -lt 0) { $done = 0 }
        ...
        $idleIndicator = if ($Script:ConsecutiveEmptyPolls -gt 3) { " (idle)" } else { "" }
        $msg = "  Next poll in {0}s [{1}]{2}" -f $secondsLeft, $bar, $idleIndicator

        Write-Host "`r$msg" -NoNewline -ForegroundColor Gray
        Start-Sleep -Milliseconds 500
    }

    # Clear the countdown line silently (no new line output)
    Write-Host "`r".PadRight(80) -NoNewline
    Write-Host "`r" -NoNewline
}

# ----------------- FOLDER ENSURER -----------------
function New-DirectoryCached {
    param(
        [string]$Path,
        [switch]$Force
    )
    
    # OPTIMIZATION: Cache folder existence to reduce filesystem calls
    if ($Script:FolderCache.ContainsKey($Path)) {
        return  # Already exists or was just created
    }
    ...
    # Cache it so we don't check again this session
    $Script:FolderCache[$Path] = $true
}
...
function Invoke-FtpDownload {
    ...
    # OPTIMIZATION: Flush logs before starting FTP operations
    Flush-LogBuffer
    ...
    try {
        ...
        try {
            ...
            foreach ($rf in $session.EnumerateRemoteFiles($normalizedRoot, "*", [WinSCP.EnumerationOptions]::AllDirectories)) {
                ...
                # OPTIMIZATION: Use cached folder creation
                if (-not $Script:FolderCache.ContainsKey($localDir)) {
                    if (-not (Test-Path $localDir)) {
                        New-Item -ItemType Directory -Path $localDir -Force | Out-Null
                        Log "Created local download subdirectory: $localDir"
                    }
                    $Script:FolderCache[$localDir] = $true
                }
                ...
                # OPTIMIZATION: Reset idle counter when files are downloaded
                $Script:ConsecutiveEmptyPolls = 0
            ...
            if (-not $anyFileEnumerated) {
                # Status messages removed to prevent scrolling - info in HUD and logs only
                Log "FTP: no files found under '$normalizedRoot'."
                # OPTIMIZATION: Track consecutive empty polls for adaptive polling
                $Script:ConsecutiveEmptyPolls++
            }
...
# OPTIMIZATION: Register cleanup handler for graceful shutdown
    ...
    Register-EngineEvent PowerShell.Exiting -Action {
        try {
            ...
            if ($Script:LogBuffer -and $Script:LogBuffer.Count -gt 0) {
                Add-Content -LiteralPath $LogFilePath -Value $Script:LogBuffer -ErrorAction SilentlyContinue
            }
        }
        catch { }
    } | Out-Null
...
# OPTIMIZATION: Hide cursor for cleaner UI from the start
try {
    [Console]::CursorVisible = $false
}
catch { }
...
while ($true) {
    try {
        ...
        # Blank line removed to prevent scrolling in static HUD
        ...
        # OPTIMIZATION: Flush log buffer after each poll cycle
        Flush-LogBuffer
